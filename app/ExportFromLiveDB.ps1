...
function Log-Message {
    ...
    try {
        ...
        # Ignore logging errors to file if file is locked, etc.
    }
}

# Clear previous log if it's a new run session (optional, but cleaner)
...
$defaultDateStr = Read-Host "Введите дату для экспорта (формат dd-MM-yyyy) [По умолчанию: $todayStr]"
...
try {
    ...
    Log-Message -Message "Неверный формат даты! Используется текущая дата: $todayStr" -Color "Yellow"
...
function Convert-CsvToExcel {
    ...
    try {
        ...
        # Using Start-Process with single string argument to ensure proper quoting handling in PS 5.1
        $argString = '-o "{0}" "{1}"' -f $ExcelPath, $CsvPath
...
function Format-ExcelFile {
    ...
    try {
        ...
        Log-Message -Message "  -> Success: Formatted Excel columns (CheckDate & PatientName widths increased)." -Color "Green"
    }
    catch {
    ...
    if ($count -eq 0) {
        ...
        if ($dtMax.Rows.Count -gt 0) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($latestDateRaw)) {
                ...
                Log-Message -Message "Переключение на последнюю доступную дату: $targetDateStr" -Color "Cyan"
            } else {
...
# Use explicit string concatenation to avoid parser confusion with quotes inside quotes if any
$reportQuery = "SELECT " +
...
Log-Message -Message "=== Finished! Files are in: $ExportFolder ===" -Color "Cyan"
